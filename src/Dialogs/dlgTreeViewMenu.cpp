...
class TreeViewMenuButtonRenderer final : public ButtonRenderer {
...
public:
  explicit TreeViewMenuButtonRenderer(const DialogLook &_look,
...
unsigned
TreeViewMenuButtonRenderer::GetMinimumButtonWidth() const noexcept
{
  return 2 * Layout::GetTextPadding() + look.button.font->TextSize(caption).width;
}

void
TreeViewMenuButtonRenderer::DrawButton(Canvas &canvas, const PixelRect &rc,
...
class TreeViewMenu final : public WindowWidget {
  WndForm &dialog;
  const Menu &menu;
...
public:
  ...
  TreeViewMenu(WndForm &_dialog, const Menu &_menu) noexcept
...
void
TreeViewMenu::Prepare(ContainerWindow &parent, const PixelRect &rc) noexcept
{
  ...
  for (unsigned i = 0; i < menu.MAX_ITEMS; ++i) {
    ...
    auto &button = buttons.emplace_back(*grid_view, button_rc, buttonStyle,
                                        std::make_unique<TreeViewMenuButtonRenderer>(dialog_look,
                                                                                  expanded.text),
                                        [this, &menuItem](){
...
bool
TreeViewMenu::KeyPress(unsigned key_code) noexcept
{
  auto &grid_view = GetWindow();
...
static int
ShowTreeViewMenu(UI::SingleWindow &parent, const Menu &menu) noexcept
{
  const auto &dialog_look = UIGlobals::GetDialogLook();

  TWidgetDialog<TreeViewMenu> dialog(WidgetDialog::Full{},
                                  parent,
                                  dialog_look, nullptr);
...
{
  const auto *menu = InputEvents::GetMenu(_T("TreeViewMenu"));
  if (menu == nullptr)
    return;

  const int event = ShowTreeViewMenu(parent, *menu);
  if (event >= 0)
